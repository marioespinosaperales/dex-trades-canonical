...
The public Evidence snapshot is a **live Alchemy warehouse export** (`source_kind=warehouse`)
across Eth / Base / Arb / Avalanche × Uniswap + Camelot / Aerodrome / Pharaoh.
Refresh locally with `make backfill && make enrich-blocks && make transform && make snapshot`
(or `make seed-dashboard` for an offline multi-pool demo).
